...
class GP5File(gp4.GP4File):
    ...
    def readDuration(self, flags: int) -> 'gp.Duration':
        ...
        The duration value is derived from a signed byte: actual_value = 1 << (raw_byte + 2).
        N-tuplets are read as a single integer which maps to specific 'enters' and 'times' values.
        """
        ...
        is_dotted = bool(flags & 0x01)  # Bit 0 of GP5 beat flags indicates dotted note
        
        tuplet_obj = gp.Tuplet()  # Default: no tuplet (1:1)
        if flags & 0x20:  # Bit 5 of GP5 beat flags indicates n-tuplet
